...
#### Web Server
- Handles **HTTP requests** and delivers **static files** (HTML, CSS, JS, images).  
- Can forward requests for dynamic content to another server.  
- Examples: Nginx, Apache, Caddy, IIS.  
👉 Like a **waiter**: quickly serves ready-made items.

---

#### Application Server
- Runs **application code** and generates **dynamic content**.  
- Connects to databases, applies business logic, and prepares responses.  
- Examples: Node.js (Express), Java Spring Boot, Django, Flask, Rails.  
👉 Like a **kitchen**: cooks custom meals when requested.
...
3. **Application server** processes, queries DB, builds result.  
4. Sends result back to **web server**, which delivers it to the client.  
